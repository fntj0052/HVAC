...
int wet = 1;
int Rpin = 0;
char email[50] = "hvacuser@gmail.com";
float temp;
float moisture;
short sound;
int result;
...
int sendData(char *email, float temp, float moisture, int noise){

      //char request[] = "GET /hvacWS/sensors/read?from=now&to=now\r\n";
      //char request[] = "GET /hvac/read.php HTTP/1.1\r\nHost: www.justlikerav.com:80\r\n\r\n";

      char requestformat[] = "GET /hvac/datatransfer.php?email=%s&temp=%f&moisture=%f&sound=%d HTTP/1.1\r\nHost: www.justlikerav.com:80\r\n\r\n";
      sprintf(request, requestformat, email, temp, moisture, noise);
	write(fd2, request, strlen(request)); // write(fd, char[]*, len);  
	/*
      bzero(buffer, BUFFER_SIZE);
	while(read(fd2, buffer, BUFFER_SIZE - 1) != 0){
...
void *myThreadFun(void *vargp)
{
            while(1)
            {
                  ...
                  //{
                  printf("\nSending DATA\n");
                  result = sendData(email, temp, moisture, noise);
                  if (result == 0)
...
//	printf("%d\n", ++step);
	...
	fd2 = socket_connect(argv[1], atoi(argv[2]));
	printf("\n Debug: %d\n   ", fd2);
      ...
      while(1)
      {
		...
		if(counter == step ) {
			...
			if ((value1 < offset) || (value2 < offset)) {
				...
				if(noise) {
					printf("\n\t%s\tNOISE !!!!  #1:%3d #2:%3d Temperature: %4.2f Moisture: %d", hvacStatus, value1, value2, temperature, moistureread);
					//both microphones are sensing noise - red 
					digitalWrite (0, LOW);
					digitalWrite (1, HIGH);
				}
				else {
					printf("\n\t%s\tVoice In!!  #1:%3d #2:%3d Temperature: %4.2f Moisture: %d", hvacStatus, value1, value2, temperature, moistureread);
